Feature('@first Create Todos 128 @step:06 @smoke @story:12345')
 ...
 * Happy Path tests
 */
Scenario('Create a new todo item 128', async ({ I, TodosPage }) => {
...
Scenario('Create multiple todo items 128', async ({ I, TodosPage }) => {
  I.say('Given I have an empty todo list')
...
Data(examples).
Scenario('Todos containing weird characters 128', async ({ I, current, TodosPage }) => {
...
Scenario('Text input field should be cleared after each item 128', async ({ I, TodosPage }) => {
  I.say('Given I have an empty todo list')
...
Scenario('Text input should be trimmed 128', async ({ I, TodosPage }) => {
  I.say('Given I have an empty todo list')
...
Scenario('New todos should be added to the bottom of the list 128', async ({ I, TodosPage }) => {
...
Scenario('Footer should be visible when adding TODOs 128', async ({ I, TodosPage }) => {
